...
try:
    from . import (config, download, coverage, clean, eda, pca, clustering,
                   compare_years, sensitivity)
except ImportError:
    import config, download, coverage, clean, eda, pca, clustering, compare_years, sensitivity
...
def main(argv=None):
    ...
    if force_dl or not snapshot.exists():
        print(">> [1/8] Descarga de datos WDI")
        download.main()
    else:
        print(">> [1/8] Snapshot crudo encontrado; se omite descarga "
              "(usar --download para forzar).")

    print("\n>> [2/8] Análisis de cobertura")
    ...
    print("\n>> [4/8] EDA (univariado + bivariado + categóricas)")
    eda.main()

    print(f"\n>> [5/8] PCA ({config.YEAR_MODERN})")
    pca.run(config.YEAR_MODERN)

    print(f"\n>> [6/8] Clustering ({config.YEAR_MODERN})")
    clustering.run(config.YEAR_MODERN)

    print(f"\n>> [7/8] Comparación temporal {config.YEAR_EARLY} vs {config.YEAR_MODERN}")
    compare_years.run()
    ...
    print(f"PIPELINE COMPLETO en {time.time()-t0:.0f}s. "
          f"Resultados en data/processed/ y reports/figures/.")
